...
const PharmaForm = ({state}) => {

  const [error, setError] = useState("");
  const [showError, setShowError] = useState(false);

  const pharmaRegister = async(event)=>{
    ...
    try {
      const transaction = await contract.addPharmacy(_name, _location, _contact, license);
      await transaction.wait();
      console.log("Transaction is done.");
      document.getElementById("register-form").reset();
      setError("");
      setShowError(false);
      window.location.href = "/PharmaEntry";
    } catch (error) {
      setError("Already registered as Pharma or an error occurred.");
      setShowError(true);
    }
  }
    ...
    <>
      <Container>
        <FormWrap>
          <Icon to="/">MRS</Icon>
          <FormContent>
            <Form id="register-form" action="/PharmaEntry" onSubmit={pharmaRegister}>
